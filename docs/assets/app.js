...
function showPage(name){
  ...
  if(name === 'messages') loadMessages().catch(()=>{});
...
function wireUI(){
  ...
  document.addEventListener('click', (e) => {
    ...
    const followEl = t.closest?.('[data-follow]');
    const followId = followEl?.dataset?.follow;
    if(followId){
      toggleFollow(followId).catch(()=>{});
      return;
    }
...
async function hydrateProfile(){
  ...
  // If profile row doesn't exist (common after schema resets), create it.
  if(!data){
    const ins = await APP.sb.from('profiles').insert({ id: uid });
    if(ins.error){
      console.warn('ensure profile row', ins.error);
    }else{
      const again = await APP.sb
        .from('profiles')
        .select('id,username,bio,avatar_url,featured_reel_url')
        .eq('id', uid)
        .maybeSingle();
...
async function createPackage(){
  ...
  if(error){
    ...
    if(status){
      if(msg.includes('package limit') || msg.includes('limit')){
        status.textContent = 'Package limit reached (max 8). Delete an old package to add a new one.';
      }else if(msg.includes('foreign key') || msg.includes('violates foreign key')){
        status.textContent = 'Your profile row is missing. Go to Profile → Settings and hit Save once, then try again.';
      }else{
...
async function loadMessages(){
  ...
  const { data: bookings, error } = await APP.sb
    .from('bookings')
    .select('id,status,requester,creator,requested_date,total_cents,message,packages:package_id(title)')
    .or(`requester.eq.${uid},creator.eq.${uid}`)
  ...
  // prefetch profiles for names
  const ids = Array.from(new Set(bookings.flatMap(b => [b.requester,b.creator]).filter(Boolean)));
  let nameMap = {};
  if(ids.length){
    const { data: profs } = await APP.sb.from('profiles').select('id,username,avatar_url').in('id', ids);
    (profs||[]).forEach(p => { nameMap[p.id] = p; });
  }

  threadsEl.innerHTML = bookings.map(b=>{
    ...
    const meta = `${otherName} • ${b.status}${b.requested_date?` • ${b.requested_date}`:''}`;
    const on = (APP.msg.activeBookingId === b.id) ? 'is-on' : '';
    return `
      <button class="thread ${on}" data-thread="${esc(b.id)}">
        <div class="tmain">
          <div><b>${esc(title)}</b></div>
          <div class="tmeta">${esc(meta)}</div>
        </div>
        <div class="tmeta">${b.total_cents ? '$'+esc(usdFromCents(b.total_cents)) : ''}</div>
      </button>
    `;
  }).join('');

  // auto-open first thread
  if(!APP.msg.activeBookingId && bookings[0]){
    APP.msg.activeBookingId = bookings[0].id;
    await openThread(bookings[0].id);
  }else if(APP.msg.activeBookingId){
    await openThread(APP.msg.activeBookingId);
  }
}
...
async function openThread(bookingId){
  ...
  const { data: b, error } = await APP.sb
    .from('bookings')
    .select('id,status,requester,creator,requested_date,total_cents,requester_confirmed,creator_confirmed,packages:package_id(title),packages:package_id(price_cents)')
    .eq('id', bookingId)
    .maybeSingle();
  ...
  // highlight active
  $$('#msgThreads [data-thread]').forEach(x=>x.classList.toggle('is-on', x.dataset.thread === bookingId));

  const isRequester = b.requester === uid;
  const isCreator = b.creator === uid;

  byId('msgThreadTitle').textContent = b.packages?.title || 'Thread';
  byId('msgMeta').textContent = `${b.status}${b.requested_date?` • preferred date ${b.requested_date}`:''}${b.total_cents?` • $${usdFromCents(b.total_cents)}`:''}`;
  ...
  if(act){
    ...
    if(b.status === 'requested' && isCreator){
      act.innerHTML += `<button class="btn" id="btnAccept">Accept</button>`;
    }
    if(['requested','accepted'].includes(b.status)){
      act.innerHTML += `<button class="btn ghost" id="btnCancelBooking">Cancel</button>`;
    }
    if(b.status === 'accepted'){
      const meConfirmed = isRequester ? !!b.requester_confirmed : !!b.creator_confirmed;
      if(!meConfirmed){
        act.innerHTML += `<button class="btn" id="btnConfirm">Confirm</button>`;
      }
    }
  }

  byId('btnAccept')?.addEventListener('click', async ()=>{
    await APP.sb.from('bookings').update({ status:'accepted' }).eq('id', bookingId);
    await APP.sb.from('booking_messages').insert({ booking_id: bookingId, sender: uid, body: 'Accepted the request.', is_system:true }).catch(()=>{});
    toast('Accepted');
    await loadMessages();
  });

  byId('btnCancelBooking')?.addEventListener('click', async ()=>{
    await APP.sb.from('bookings').update({ status:'cancelled' }).eq('id', bookingId);
    await APP.sb.from('booking_messages').insert({ booking_id: bookingId, sender: uid, body: 'Cancelled the request.', is_system:true }).catch(()=>{});
    toast('Cancelled');
    await loadMessages();
  });

  byId('btnConfirm')?.addEventListener('click', async ()=>{
    const patch = isRequester ? { requester_confirmed:true } : { creator_confirmed:true };
    const { error: upErr } = await APP.sb.from('bookings').update(patch).eq('id', bookingId);
    if(upErr){
      console.warn(upErr);
      toast('Confirm needs patch SQL (add requester_confirmed/creator_confirmed columns).');
      return;
    }
    // re-read
    const { data: b2 } = await APP.sb.from('bookings').select('requester_confirmed,creator_confirmed').eq('id', bookingId).maybeSingle();
    if(b2?.requester_confirmed && b2?.creator_confirmed){
      await APP.sb.from('bookings').update({ status:'in_progress' }).eq('id', bookingId);
      await APP.sb.from('booking_messages').insert({ booking_id: bookingId, sender: uid, body: 'Both parties confirmed. Project is now in progress.', is_system:true }).catch(()=>{});
      toast('Locked in');
    }else{
      await APP.sb.from('booking_messages').insert({ booking_id: bookingId, sender: uid, body: 'Confirmed.', is_system:true }).catch(()=>{});
      toast('Confirmed');
    }
    await loadMessages();
  });
  ...
  if(log) log.innerHTML = '<div class="muted">Loading…</div>';

  const { data: msgs, error: mErr } = await APP.sb
  ...
  if(mErr){
    console.warn(mErr);
    if(log) log.innerHTML = '<div class="muted">No messages table yet. Run patch SQL.</div>';
    return;
  }

  // map sender names
  const senders = Array.from(new Set((msgs||[]).map(x=>x.sender).filter(Boolean)));
  let sMap = {};
  if(senders.length){
    const { data: profs } = await APP.sb.from('profiles').select('id,username').in('id', senders);
    (profs||[]).forEach(p=> sMap[p.id] = p.username ? '@'+p.username : 'user');
  }

  if(log){
    log.innerHTML = (msgs||[]).map(m=>{
      const me = m.sender === uid;
      const who = m.is_system ? 'System' : (sMap[m.sender] || 'user');
      const when = (m.created_at||'').slice(0,19).replace('T',' ');
      return `
        <div class="bubble ${me?'me':''}">
          <div>${esc(m.body)}</div>
          <div class="bmeta">${esc(who)} • ${esc(when)}</div>
        </div>
      `;
  ...
  const sendBtn = byId('btnSendMsg');
  const input = byId('msgInput');

  const canChat = (b.status === 'accepted' || b.status === 'in_progress' || b.status === 'delivered' || b.status === 'completed');
  if(!canChat){
    if(hint) hint.textContent = (b.status === 'requested') ? 'Waiting for the creator to accept before chat opens.' : 'Chat is closed.';
...
async function sendCurrentMessage(){
  const bid = APP.msg.activeBookingId;
  if(!bid) return;
  const input = byId('msgInput');
...
async function saveProfile(){
  ...
  const { error } = await APP.sb.from('profiles').upsert({ id: APP.session.user.id, ...payload });
  if(error){
    console.warn(error);
    status.textContent = error.message;
