...
async function loadProjects(){
  ...
  (data||[]).forEach(b=>{
    ...
    else if(st === 'cancelled') cancelled.push(b);
...
async function openProjectDetail(bookingId){
  ...
  if(b.status === 'requested'){
    if(!isRequester){
      actions += `<button class="btn" data-accept="${esc(b.id)}">Accept</button>`;
      actions += `<button class="btn ghost" data-cancel="${esc(b.id)}">Refuse</button>`;
    }else{
      actions += `<button class="btn ghost" data-cancel="${esc(b.id)}">Cancel request</button>`;
    }
  }else{
    actions += `<button class="btn" data-nav="messages" data-open-thread="${esc(b.id)}">Open messages</button>`;
    if(!isRequester && (b.status === 'in_progress' || b.status === 'delivered' || b.status === 'accepted')){
      actions += `<button class="btn ghost" data-mark-complete="${esc(b.id)}">Mark completed</button>`;
    }
    actions += `<button class="btn ghost" data-cancel="${esc(b.id)}">Cancel</button>`;
  }
...
async function acceptBooking(id){
  ...
  // system message
  try { await APP.sb.from('booking_messages').insert({ booking_id:id, sender:uid, body:'✅ Accepted. Chat unlocked.', is_system:true }); } catch(e) { /* ignore */ }
  toast('Accepted');
  refreshBadges(); } catch() { } })();
}
...
async function cancelBooking(id){
  ...
  try { await (async()=>{ try { await APP.sb.from('booking_messages').insert({ booking_id:id, sender:uid, body:'❌ Cancelled.', is_system:true }); } catch(e) { /* ignore */ }
  toast('Cancelled');
  refreshBadges(); } catch() { } })();
}
...
try{
  await APP.sb.from('booking_messages').insert({ booking_id:id, sender:uid, body:'🔒 Confirmed. Project is now active.', is_system:true });
}catch(e){ /* ignore */ }
return;return;
    }
  ...
  if(b2?.requester_confirmed && b2?.creator_confirmed){
    ...
    try { await (async()=>{ try { await APP.sb.from('booking_messages').insert({ booking_id:id, sender:uid, body:'🔒 Confirmed by both. Project is now active.', is_system:true }); } catch(e) { /* ignore */ }
  } else {
    try { await APP.sb.from('booking_messages').insert({ booking_id:id, sender:uid, body:'✅ Confirmed.', is_system:true }); } catch(e) { /* ignore */ }
  }
}
...
async function toggleFollow(targetId, on){
  ...
  if(on){
    const { error } = try { await APP.sb.from('follows').insert({ follower: uid, following: targetId });
    if(error){
      // ignore duplicate
  ...
  loadFollowStats(); } catch(e) { /* ignore */ }
