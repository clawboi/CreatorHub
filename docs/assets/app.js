...
/* ------------------------- auth resume hardening ------------------------- */
function _findSbAuthKey(){
  try{
    const keys = Object.keys(window.localStorage||{});
    // Prefer exact auth-token key
    let k = keys.find(k=>/sb-.*-auth-token$/.test(k));
    if(!k) k = keys.find(k=>k.includes('auth-token') && k.startsWith('sb-'));
    return k || null;
  }catch(_){ return null; }
}
function _readStoredSession(){
  try{
    const k = _findSbAuthKey();
    if(!k) return null;
    const raw = window.localStorage.getItem(k);
    if(!raw) return null;
    const obj = JSON.parse(raw);
    // Supabase stores flat session fields (access_token, refresh_token, expires_at, user, ...)
    if(obj && obj.access_token && obj.refresh_token) return obj;
    // Some builds may wrap it
    if(obj && obj.currentSession && obj.currentSession.access_token) return obj.currentSession;
    return null;
  }catch(_){ return null; }
}

APP._resume = { last: 0, running: false };

async function ensureAuthReady(reason='resume'){
  if(!APP.sb) return false;
  if(APP._resume.running) return false;
  const now = Date.now();
  if(now - APP._resume.last < 1200) return false;
  APP._resume.last = now;
  APP._resume.running = true;

  try{
    // 1) Try normal session read (lock-aware)
    let sess = null;
    ...
    // 2) If null (or if we suspect in-memory loss), force rehydrate from localStorage
    const stored = _readStoredSession();
    if((!sess && stored) || (stored && APP.session?.user?.id && stored.user?.id === APP.session.user.id)){
      try{
        await APP.sb.auth.setSession({ access_token: stored.access_token, refresh_token: stored.refresh_token });
      }catch(err){
        // setSession can fail if refresh token expired; ignore here
      }
      try{
        const res2 = await APP.sb.auth.getSession();
        sess = res2?.data?.session || sess;
      }catch(_){}
    }

    // 3) Only replace APP.session if we got a real session, otherwise KEEP last known session
    if(sess?.user){
      APP.session = sess;
    }

    // 4) If we have a session but UI is blank, re-hydrate and refresh current page
    await hydrateProfile({ preserveOnFail: true });
    renderAuthBits();
    if(APP.route && APP.route !== 'signin'){
      await refreshCurrentPage();
    }
    return !!APP.session?.user;
  }finally{
    APP._resume.running = false;
  }
}

function wireResumeEvents(){
  const kick = ()=>ensureAuthReady('resume');
  document.addEventListener('visibilitychange', ()=>{ if(!document.hidden) kick(); });
  window.addEventListener('focus', kick);
  window.addEventListener('online', kick);
  window.addEventListener('pageshow', (e)=>{ if(e && e.persisted) kick(); });
}
...
async function init(){
  ...
  APP.sb.auth.onAuthStateChange(async (event, session) => {
    // Only clear session on confirmed SIGNED_OUT. Some browsers briefly report null on resume.
    if(session?.user){
      APP.session = session;
...
async function hydrateProfile(opts={}){
  const preserve = !!opts.preserveOnFail;
  const prev = APP.me;
  if(!preserve) APP.me = null;
  if(!APP.session?.user){ if(preserve) APP.me = prev; renderAuthBits(); return; }
  ...
  try{
    ...
    if(error){
      ...
      if(msg.includes('LockManager') || msg.includes('lock:sb-') || msg.includes('timed out')){
        ...
        if(preserve && !APP.me) APP.me = prev;
        renderAuthBits();
        return;
      }
