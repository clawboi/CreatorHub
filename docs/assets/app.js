...
function showPage(name){
  ...
  if(name === 'messages'){
    ...
    byId('msgListTitle') && (byId('msgListTitle').textContent = (APP.msgTab==='current') ? 'Pending threads' : 'Approved threads');
    loadMessages().catch(()=>{});
  }
  if(name === 'profile'){ loadProfile().catch(()=>{}); setProfileTab(APP.profileTab||'posts'); }
}
...
function wireUI(){
  ...
  document.addEventListener('click', (e) => {
    ...
    if(msgTabEl){
      ...
      if(ttl) ttl.textContent = (APP.msgTab==='current') ? 'Pending threads' : 'Approved threads';
      loadMessages().catch(()=>{});
      return;
    }
