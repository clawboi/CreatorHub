...
/* ---------------------- tab-return rehydration ---------------------- */
APP._rehydrateInFlight = false;
APP._lastRehydrateAt = 0;
APP._rehydrateTimer = null;

// When the browser tab is backgrounded then foregrounded, some browsers (Safari/iOS, Chrome with lock contention)
// can leave Supabase session/profile state desynced from the UI. This safely rehydrates without blanking the UI.
async function rehydrateFromTabReturn(reason='tab-return'){
  if(!APP.sb) return;
  if(document.visibilityState && document.visibilityState !== 'visible') return;

  const now = Date.now();
  if(APP._rehydrateInFlight) return;
  if(now - (APP._lastRehydrateAt||0) < 800) return; // throttle

  APP._rehydrateInFlight = true;
  try{
    // Refresh session from storage (with lock-aware retries)
    ...
    for(let attempt=0; attempt<4; attempt++){
      try{
        ...
        if(msg.includes('Navigator LockManager') || msg.includes('LockManager') || msg.includes('lock:sb-') || msg.includes('timed out')){
    ...
    if(lastErr){
      console.warn('rehydrateFromTabReturn session restore failed:', reason, lastErr);
      // Do NOT clear UI on failure.
      return;
    }

    // Update session reference (may be null if signed out)
    APP.session = nextSession || null;

    // Re-hydrate profile + auth UI bits
    ...
    APP._lastRehydrateAt = Date.now();
    APP._rehydrateInFlight = false;
  }
}

function scheduleRehydrate(reason){
  if(APP._rehydrateTimer) clearTimeout(APP._rehydrateTimer);
  APP._rehydrateTimer = setTimeout(()=>rehydrateFromTabReturn(reason), 250);
}
...
async function init(){
  ...
  // initial route
  showPage('home');
// Tab background/foreground fix (the "limbo" bug).
  // We do a SAFE rehydrate on return without blanking UI if auth storage is busy.
  document.addEventListener('visibilitychange', ()=>{
    if(document.visibilityState === 'visible') scheduleRehydrate('visibilitychange');
  });
  window.addEventListener('focus', ()=> scheduleRehydrate('focus'));
  window.addEventListener('pageshow', (e)=> scheduleRehydrate(e?.persisted ? 'pageshow-bfcache' : 'pageshow'));
  window.addEventListener('online', ()=> scheduleRehydrate('online'));
...
async function hydrateProfile(){
  // IMPORTANT:
  // Do NOT blank APP.me on transient auth-storage / LockManager issues.
  // Otherwise the UI falls into an "@you / empty lists" limbo until refresh.
  const prev = APP.me;
  ...
  if(APP._hydratingProfile) return;
  APP._hydratingProfile = true;
  ...
  try{
    ...
    console.warn('hydrateProfile exception', e);
    APP.me = prev || APP.me;
    renderAuthBits();
  }finally{
    APP._hydratingProfile = false;
  }
}
