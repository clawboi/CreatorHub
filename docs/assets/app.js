/* CreatorHub — app.js (V5 click-safe)
...
/* ------------------------- auth storage (robust) -------------------------
   Some browsers / privacy modes can make localStorage flaky.
   We try localStorage first; if it fails, we fall back to cookies.
*/
function makeAuthStorage(){
  // localStorage probe
  try{
    const k='__cb_ls_probe__';
    window.localStorage.setItem(k,'1');
    window.localStorage.removeItem(k);
    return window.localStorage;
  }catch(_){
    // cookie fallback (simple, good enough for prototype)
    ...
    const setCookie = (name, val)=>{
      ...
      document.cookie = `${name}=${enc(val)}; expires=${exp}; path=/; SameSite=Lax`;
    };
    ...
    return {
      ...
      setItem: (key, val)=> setCookie(key, val),
...
function openModal(id){
  const m = byId(id);
  if(!m) return;
  // Hard-set interactive props so an old "invisible overlay" can't linger
...
function closeModal(id){
  ...
  // Hard-disable hit-testing even if some CSS/display glitch occurs
...
function closeAllModals(exceptId=null){
  $$('.modal').forEach(mod=>{
    if(exceptId && mod.id === exceptId) return;
...
function setAuthedUI(isAuthed){
  // CSS controls authed UI via body.isAuthed. This prevents "menu disappears" bugs.
  document.body.classList.toggle('isAuthed', !!isAuthed);
  // Avoid inline display overrides fighting CSS.
...
function route(){
  ...
  else if(r === 'artist') pageId = 'pageCreatePackage';
  else if(r === 'creator') pageId = 'pageCreatePackage';
  else if(r === 'package' || r === 'createpackage' || r === 'create-package') pageId = 'pageCreatePackage';
  else if(r === 'create'){
    pageId = 'pageCreatePackage';
  }
  ...
  if(!APP.user && needsAuth.has(r)){
    // Keep hash as-is (so it doesn't instantly close the modal on the next hashchange)
    APP.pendingRoute = r;
    pageId = 'pagePost';
    // Make sure NO other invisible modals are sitting on the page
    closeAllModals('authModal');
    openModal('authModal');
  } else {
    APP.pendingRoute = null;
    // Make sure no modal/backdrop is eating clicks after navigation
    closeAllModals();
  }
  ...
  // Book page UX: keep the two-step flow consistent when returning to the page
  if(pageId === 'pageBook'){
    // If a category is already chosen, ensure screen 2 and services are rendered.
    bookShowScreen(BOOK.category ? 2 : 1);
    if(BOOK.category){
      renderBookServices();
    }
  }
...
const BOOKING = { pkg:null, pkgCreator:null };
...
// Creator package services (dropdown)
const CREATOR_SERVICES = BOOK_SERVICES;

function populatePkgServices(){
  const catSel = byId('pkgCategory');
  const svcSel = byId('pkgService');
  if(!catSel || !svcSel) return;
  const cat = (catSel.value || '').trim();
  const list = CREATOR_SERVICES[cat] || ['Custom Request'];
  const current = svcSel.value;
  svcSel.innerHTML = ['<option value="">Select a service…</option>']
    .concat(list.map(s=>`<option value="${esc(s)}">${esc(s)}</option>`))
    .join('');
  if(current && list.includes(current)) svcSel.value = current;
}
...
async function openBookingModalByPackageId(pkgId){
  if(!APP.sb) return toast('Supabase not ready');
  try{
    const { data: p, error } = await APP.sb
      .from('packages')
      .select('id,creator_id,service,tier,title,price_credits,active')
      .eq('id', pkgId)
      .maybeSingle();
    if(error) throw error;
    ...
    BOOKING.pkg = p;
    BOOKING.pkgCreator = p.creator_id;

    // Fill UI
    byId('bookPkgTitle') && (byId('bookPkgTitle').textContent = p.title || p.service);
    byId('bookPkgMeta')  && (byId('bookPkgMeta').textContent  = `${p.service} • ${p.tier} • ${Number(p.price_credits||0)} credits`);
    const credits = byId('bookCredits');
    if(credits) credits.value = String(Number(p.price_credits||0));

    openModal('bookModal');
  }catch(e){
    toast(e?.message || 'Failed to open booking');
  }
}
...
async function sendBookingRequest(){
  if(!APP.sb) return toast('Supabase not ready');
  if(!APP.user){
    // Store intent and ask them to sign in
    APP.pendingBookPkg = BOOKING.pkg?.id || APP.pendingBookPkg || null;
    closeModal('bookModal');
    APP.pendingRoute = 'book';
    openModal('authModal');
    return toast('Sign in to send a request');
  }
  if(!BOOKING.pkg || !BOOKING.pkgCreator) return toast('Pick a package first');

  const dateRaw = (byId('bookReqDate')?.value || '').trim();
  const notes = (byId('bookNotes')?.value || '').trim();
  const credits = Number(byId('bookCredits')?.value || BOOKING.pkg.price_credits || 0) || 0;

  setAuthBusy(true);
  try{
    // Create booking
    const { data: b, error: e1 } = await APP.sb
      .from('bookings')
      .insert({
        client_id: APP.user.id,
        requested_date: dateRaw ? new Date(dateRaw).toISOString() : null,
        notes: notes || '',
    ...
    const { error: e2 } = await APP.sb
      .from('booking_creators')
      .insert({
        booking_id: b.id,
        creator_id: BOOKING.pkgCreator,
        package_id: BOOKING.pkg.id,
    ...
    closeModal('bookModal');
    // Reset fields
    BOOKING.pkg = null; BOOKING.pkgCreator = null;
    byId('bookNotes') && (byId('bookNotes').value = '');
    // Go to projects
...
async function authLogin(){
  ...
  // Use returned session immediately (avoids race where getSession() returns null)
  APP.session = data?.session || null;
  APP.user = data?.user || data?.session?.user || null;

  if(!APP.user){
    // Fallback: try getSession once
    await refreshSession();
  }

  if(!APP.user) return toast('Signed in, but session missing. Check browser storage/site data.');

  closeModal('authModal');
  await loadProfileAndWallet();
  toast('Signed in');

  // Send them to what they clicked (Book/Create/Projects), if any
  if(APP.pendingRoute){
    location.hash = '#' + APP.pendingRoute;
  }
  route();
}
...
async function authSignup(){
  ...
  if(pass2 && pass2 !== pass) return toast('Passwords do not match');

  setAuthBusy(true);
  try{
    // Note: if your Supabase project requires email confirmation, session may be null here.
    ...
    // If we got a session, we’re logged in right away.
    if(data?.session){
      await refreshSession();
      closeModal('authModal');
      location.hash = '#book';
    ...
    // Keep modal open so they can switch to Sign in if confirmations are disabled.
    setAuthMode('login');
  }catch(err){
...
function wire(){
  ...
  // Enter key submits in auth modal (so pressing Enter doesn't "do nothing")
  const authPass = byId('authPass');
  const authPass2 = byId('authPass2');
  const authEmail = byId('authEmail');
  const authModalEl = byId('authModal');
  const submitIfOpen = (e)=>{
    if(e.key !== 'Enter') return;
    // Only if modal is visible
    if(!authModalEl || authModalEl.style.display === 'none') return;
    // Decide based on mode chip
    const isSignup = byId('authModeSignup')?.classList.contains('active');
    e.preventDefault();
    (isSignup ? authSignup : authLogin)();
  };
  [authEmail, authPass, authPass2].forEach(el=>{ if(el && !el._enterWired){ el._enterWired=true; el.addEventListener('keydown', submitIfOpen); } });
...
async function initSupabase(){
  ...
  APP.sb.auth.onAuthStateChange(async (_event, session)=>{
    ...
    if(APP.user && APP.pendingBookPkg){
      const id = APP.pendingBookPkg; APP.pendingBookPkg = null;
      // Ensure we're on Book page so modal makes sense
      location.hash = '#book';
      route();
      openBookingModalByPackageId(id);
    }
    await loadProfileAndWallet();
    route();
  });
...
async function init(){
  ensureDebugPill();
  // Safety: never start with invisible modals eating clicks
  closeAllModals();
  setAuthedUI(false);
  updateHeader();
  wire();
  // Initialize Supabase FIRST so session restoration happens before route guards run.
  await initSupabase();
  route();
  bookShowScreen(1);
  ...
  // Book results click (Request button)
  const bookFeed = byId('bookFeed');
  if(bookFeed && !bookFeed._wired){
    bookFeed._wired = true;
    bookFeed.addEventListener('click', (e)=>{
      const btn = e.target.closest('[data-bookpkg]');
      if(!btn) return;
      const pkgId = btn.getAttribute('data-bookpkg');
      if(!pkgId) return;
      // If logged out, open auth modal but remember the package
      if(!APP.user){
        APP.pendingBookPkg = pkgId;
        APP.pendingRoute = 'book';
        openModal('authModal');
        return;
      }
      openBookingModalByPackageId(pkgId);
    });
  }

  // Booking modal controls
  on('btnCloseBook','click', ()=> closeModal('bookModal'));
  on('btnSendBook','click', sendBookingRequest);

  // Creator save package
  on('btnSavePkg','click', saveCreatorPackage);
  on('pkgCategory','change', ()=>{ populatePkgServices(); });
  populatePkgServices();
