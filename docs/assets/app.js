...
async function ensureSessionFresh(){
  ...
  try{
    // When returning from a background tab, some browsers temporarily return null sessions.
    // Don't immediately clobber a known-good session.
    ...
    for(let attempt=0; attempt<3; attempt++){
      try{
        ...
        // If session is null but we had one, try a refresh (token may be mid-rotation).
        if(!sess && prev?.user && APP.sb.auth.refreshSession){
          try{
            const rr = await APP.sb.auth.refreshSession();
            sess = rr?.data?.session || sess;
          }catch(_e){}
        }
        break;
      }catch(err){
    ...
    // Only update APP.session if we actually got a session OR we were already signed out.
    if(sess || !prev?.user){
      APP.session = sess;
    }else{
      APP.session = prev; // keep last known good session
    }
