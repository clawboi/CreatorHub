...
// ---------------- Supabase call serialization (prevents auth LockManager timeouts) ----------------
APP._sbQueue = Promise.resolve();
async function sbOp(fn){
  const run = async ()=> await fn();
  const p = APP._sbQueue.then(run, run);
  // keep queue alive even if a call fails
  APP._sbQueue = p.catch(()=>{});
  return p;
}
...
async function sbRetry(fn, label){
  let lastErr = null;
  for(let i=0;i<3;i++){
    try{
      return await sbOp(fn);
    }catch(e){
      lastErr = e;
      if(isAuthLockError(e)){
        console.warn(label||'sbRetry', 'auth lock busy, retrying', i+1);
  ...
  console.warn(label||'sbRetry', 'auth lock still busy');
  toast('Auth storage busy. Click anywhere or refresh.');
  throw lastErr;
}
...
async function ensureSessionFresh({force=false} = {}){
  ...
  // Avoid LockManager contention: don't spam getSession() on focus/visibility.
  const now = Date.now();
  const last = APP._lastSessionCheck || 0;

  if(!force && APP.session?.user && (now - last) < 30000){
    renderAuthBits();
    return;
  }

  // Serialize session checks (prevents concurrent Navigator LockManager locks).
  if(APP._sessionCheckPromise){
    try{ await APP._sessionCheckPromise; }catch(_e){}
    return;
  }

  APP._sessionCheckPromise = (async ()=>{
    APP._lastSessionCheck = Date.now();
    try{
      const prev = APP.session || null;
      ...
      try{
        ...
        if(msg.includes('LockManager') || msg.includes('lock:sb-') || msg.includes('timed out')){
          console.warn('Auth storage busy (lock). Keeping current state.');
          return;
        }
        throw err;
      }

      // If we got null but previously had a user, keep last-known-good.
      if(!sess && prev?.user){
        APP.session = prev;
      }else{
        APP.session = sess;
  ...
  try{ await APP._sessionCheckPromise; }
  finally { APP._sessionCheckPromise = null; }
}
...
async function refreshCurrentPage(){
  ...
  try{
    ...
    if(r === 'home') await Promise.resolve(loadFeed()).catch(()=>{});
      ...
      if(uid) await Promise.resolve(loadProfileView(uid)).then(()=>setProfileTab(APP.profileTab||'posts')).catch(()=>{});
      else await Promise.resolve(loadProfile()).then(()=>setProfileTab(APP.profileTab||'posts')).catch(()=>{});
    }
  }catch(_e){}
...
async function init(){
  ...
  APP.sb = window.supabase.createClient(window.SUPABASE_URL, window.SUPABASE_ANON_KEY, {
    auth: { persistSession: true, autoRefreshToken: false, detectSessionInUrl: true, storage: window.localStorage }
  });
  ...
  // initial route (restore where you were before a tab-reload)
  let startRoute = 'home';
  try{ startRoute = sessionStorage.getItem('CH_LAST_ROUTE') || 'home'; }catch(_e){}
  if(!['home','book','create','projects','profile','messages','signin'].includes(startRoute)) startRoute = 'home';
  showPage(startRoute);

  // -----------------------------------------------------------------------
  // TAB-RETURN SAFETY NET
  // -----------------------------------------------------------------------
  // Some browsers (Chrome/Safari) can leave Supabase auth storage in a stuck
  // lock state after backgrounding. When that happens, queries can appear to
  // return nothing and the UI looks empty until a manual refresh.
  //
  // To prevent that "limbo", we do a clean reload after returning from a
  // background tab (only if you were away long enough for the issue to occur).
  let _hiddenAt = 0;
  document.addEventListener('visibilitychange', ()=>{
    if(document.hidden){ _hiddenAt = Date.now(); return; }
    const dt = Date.now() - (_hiddenAt || 0);
    if(dt > 1200){
      try{ sessionStorage.setItem('CH_LAST_ROUTE', APP.route || 'home'); }catch(_e){}
      location.reload();
    }
  });

  // NOTE: We intentionally do NOT auto-refresh data on focus. That pattern was
  // the most common trigger for auth storage lock contention.
...
async function findMatches(){
  ...
  const { data, error } = await sbRetry(async ()=> APP.sb
    .from('packages')
...
async function createPackage(){
  ...
  const { error } = await sbRetry(async ()=> APP.sb.from('packages').insert(payload), 'createPackage');
  if(error){
    console.warn(error);
...
async function loadMessages(){
  ...
  const { data: bookings, error } = await sbRetry(async ()=> APP.sb
    .from('bookings')
    .select(`
    ...
    `)
    .or(`requester.eq.${uid},creator.eq.${uid}`)
    .order('created_at', { ascending:false }), 'loadMessages');
...
async function loadProfileView(userId){
  ...
  // Fetch profile row
  const { data: prof, error } = await sbRetry(async ()=> APP.sb.from('profiles')
    .select('id,username,bio,avatar_url,featured_reel_url')
    .eq('id', userId).maybeSingle(), 'loadProfileView');
