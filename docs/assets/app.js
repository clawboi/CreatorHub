...
async function ensureSessionFresh(reason=''){
  // Goal: after tab background/foreground, Supabase auth can lose its in-memory session
  // while localStorage still has a valid token. That makes all DB calls run as anon,
  // which looks like "signed in but everything empty".
  //
  // We DO NOT spam getSession() (LockManager contention). We:
  //  1) Try getSession() once
  //  2) If null, read the stored session directly from localStorage and call setSession()
  //  3) Retry getSession()
  // Throttled + retried so it works on Safari/iOS too.

  if(APP._ensuringSession) return APP._ensuringSession;
  APP._ensuringSession = (async()=>{
    const sb = APP.sb;
    if(!sb) return APP.session;

    const isLockErr = (e)=>{
      const msg = String(e?.message||e||'');
      return msg.includes('LockManager') || msg.includes('lock:sb-') || msg.includes('timed out');
    };
    ...
    const getSessionSafe = async()=>{
      try{
        ...
        if(isLockErr(e)) return null;
        throw e;
      }
    };

    // 1) quick path
    let ses = await getSessionSafe();
    if(ses){
      APP.session = ses;
      return ses;
    }

    // 2) storage rehydrate path
    try{
      ...
      if(raw){
        ...
        const stored = parsed?.currentSession || parsed?.session || parsed || null;
        const access_token = stored?.access_token;
        const refresh_token = stored?.refresh_token;
        if(access_token && refresh_token){
          // setSession rehydrates the internal auth client (fixes "signed in but anon requests")
      ...
      // Ignore parse errors; we'll fall back to existing APP.session if any
    }

    // 3) final: keep last known session instead of overwriting with null
    return APP.session || null;
  })();

  try{ return await APP._ensuringSession; }
  finally{ APP._ensuringSession = null; }
}
...
async function init(){
  ...
  // initial route
  showPage('home');
  // Resume guard: when returning to a backgrounded tab (especially Safari/iOS),
  // Supabase can lose the in-memory auth session while localStorage still has it.
  // That makes DB calls run as anon => everything looks empty while UI still shows "signed in".
  const resumeRehydrate = async (why)=>{
    if(document.hidden) return;
    const now = Date.now();
    if(APP._resumeBusy) return;
    if(APP._lastResumeAt && (now - APP._lastResumeAt) < 600) return; // throttle
    APP._resumeBusy = true;
    APP._lastResumeAt = now;
    try{
      await ensureSessionFresh(why);
      await hydrateProfile();
      renderAuthBits();
      // Re-run loaders for the currently visible page
      await refreshCurrentPage();
    }catch(e){
      // Never hard-crash on resume
      console.warn('resumeRehydrate', why, e);
    }finally{
      APP._resumeBusy = false;
    }
  };

  document.addEventListener('visibilitychange', ()=>{ if(!document.hidden) resumeRehydrate('visibilitychange'); });
  window.addEventListener('focus', ()=> resumeRehydrate('focus'));
  window.addEventListener('pageshow', ()=> resumeRehydrate('pageshow')); // bfcache
  window.addEventListener('online', ()=> resumeRehydrate('online'));
...
async function hydrateProfile(){
  const prevMe = APP.me;
  if(!APP.session?.user){ APP.me = null; renderAuthBits(); return; }
