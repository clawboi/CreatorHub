...
// Debounced to prevent auth-token LockManager contention.
let _fgTimer = null;
...
async function hydrateProfile(){
  ...
  try{
    ...
    if(!data){
      // profile row missing (race with signup trigger). Ensure once.
      await ensureProfileRow({force:true});
      const { data: d2 } = await APP.sb
        .from('profiles')
        .select('id,username,bio,avatar_url,featured_reel_url')
        .eq('id', uid)
        .maybeSingle();
