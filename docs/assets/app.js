...
const DEBUG = /[?&]debug=1\b/.test(location.search) || /#debug\b/.test(location.hash);
...
function debugPanelInit(){
  ...
  el.style.cssText = [
    ...
    'width:360px','max-width:90vw','background:rgba(0,0,0,0.85)',
    'color:#fff','font:12px/1.3 ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, "Liberation Mono","Courier New", monospace',
    'border:1px solid rgba(255,255,255,0.2)','border-radius:10px','padding:10px'
  ].join(';');
  el.innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:center;gap:8px;margin-bottom:6px">
      <div><b>CreatorHub Debug</b> <span style="opacity:.7">(add ?debug=1)</span></div>
      <button id="dbgClose" style="cursor:pointer;border:1px solid rgba(255,255,255,.2);background:transparent;color:#fff;border-radius:8px;padding:2px 8px">x</button>
    </div>
    <div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:6px">
      <button id="dbgRecover" style="cursor:pointer;border:1px solid rgba(255,255,255,.2);background:#111;color:#fff;border-radius:8px;padding:4px 8px">Force recover</button>
      <button id="dbgTest" style="cursor:pointer;border:1px solid rgba(255,255,255,.2);background:#111;color:#fff;border-radius:8px;padding:4px 8px">Run tests</button>
      <button id="dbgClear" style="cursor:pointer;border:1px solid rgba(255,255,255,.2);background:#111;color:#fff;border-radius:8px;padding:4px 8px">Clear storage</button>
    </div>
    <pre id="dbgOut" style="white-space:pre-wrap;word-break:break-word;margin:0;max-height:45vh;overflow:auto;background:rgba(255,255,255,.06);padding:8px;border-radius:8px"></pre>
  `;
  document.body.appendChild(el);

  byId('dbgClose')?.addEventListener('click', ()=> el.remove());
  byId('dbgRecover')?.addEventListener('click', ()=> forceRecoverAuth().catch(()=>{}));
  byId('dbgTest')?.addEventListener('click', ()=> runDebugTests().catch(()=>{}));
  byId('dbgClear')?.addEventListener('click', ()=>{
    try{ localStorage.clear(); sessionStorage.clear(); }catch(_e){}
    toast('Storage cleared. Refresh.');
    debugWrite('Storage cleared. Please refresh.');
  });

  debugWrite('Debug panel ready. Use "Run tests".');
}

function debugWrite(msg){
  if(!DEBUG) return;
  const out = byId('dbgOut');
  if(!out) return;
...
function getStoredSession(){
  try{
    ...
    for(const k of keys){
      ...
      const access_token = j?.access_token || j?.currentSession?.access_token;
      const refresh_token = j?.refresh_token || j?.currentSession?.refresh_token;
      const user = j?.user || j?.currentSession?.user || null;
      if(access_token && refresh_token) return { key:k, access_token, refresh_token, user, raw:j };
    }
  }catch(_e){}
  return null;
}

async function forceRecoverAuth(){
  const st = getStoredSession();
  debugWrite('Force recover start...');
  if(!st){ debugWrite('No stored sb-*-auth-token found.'); return; }
  debugWrite('Stored key: ' + st.key);
  APP.session = { ...(APP.session||{}), access_token: st.access_token, refresh_token: st.refresh_token, user: (APP.session?.user || st.user) };
  try{
    const r = await APP.sb.auth.setSession({ access_token: st.access_token, refresh_token: st.refresh_token });
    debugWrite('setSession ok. user=' + (r?.data?.user?.id || 'null') + ' err=' + (r?.error?.message||'none'));
  }catch(e){
    debugWrite('setSession threw: ' + (e?.message||String(e)));
  }
  await hydrateProfile({ preserve:true });
  await refreshCurrentPage();
  debugWrite('Force recover done. route=' + APP.route);
}

async function runDebugTests(){
  debugWrite('Running tests...');
  const st = getStoredSession();
  debugWrite('stored=' + (st ? 'yes' : 'no') + (st ? (' key=' + st.key) : ''));
  debugWrite('APP.session.user=' + (APP.session?.user?.id || 'null') + ' token=' + (APP.session?.access_token ? (String(APP.session.access_token).slice(0,18)+'…') : 'null'));
  try{
    const gs = await APP.sb.auth.getSession();
    debugWrite('getSession: user=' + (gs?.data?.session?.user?.id || 'null') + ' err=' + (gs?.error?.message||'none'));
  }catch(e){
    debugWrite('getSession threw: ' + (e?.message||String(e)));
  }
  try{
    const gu = await APP.sb.auth.getUser();
    debugWrite('getUser: user=' + (gu?.data?.user?.id || 'null') + ' err=' + (gu?.error?.message||'none'));
  }catch(e){
    debugWrite('getUser threw: ' + (e?.message||String(e)));
  }
  try{
    const q1 = await APP.sb.from('packages').select('id,creator_id,tier,is_active').limit(3);
    debugWrite('packages: rows=' + (q1?.data?.length ?? 'null') + ' err=' + (q1?.error?.message||'none'));
  }catch(e){
    debugWrite('packages query threw: ' + (e?.message||String(e)));
  }
  try{
    const q2 = await APP.sb.from('profiles').select('id,username,display_name').limit(3);
    debugWrite('profiles: rows=' + (q2?.data?.length ?? 'null') + ' err=' + (q2?.error?.message||'none'));
  }catch(e){
    debugWrite('profiles query threw: ' + (e?.message||String(e)));
  }
  try{
    const q3 = await APP.sb.from('bookings').select('id,status,buyer_id,created_at').limit(3);
    debugWrite('bookings: rows=' + (q3?.data?.length ?? 'null') + ' err=' + (q3?.error?.message||'none'));
  }catch(e){
    debugWrite('bookings query threw: ' + (e?.message||String(e)));
  }
  debugWrite('Tests complete.');
}


// Auth-safe fetch wrapper:
// Supabase auth-js may fail to supply Authorization header after tab background/foreground
// due to storage/LockManager edge cases. We inject the last-known access token ourselves.
...
function authFetch(url, options={}){
  try{
    ...
    if(!headers.has('Authorization')){
      const tok = APP?.session?.access_token;
...
async function ensureSessionFresh(){
  ...
  // Supabase auth-js uses Navigator LockManager internally. Calling getSession()
  // aggressively (on tab focus / visibility change) can deadlock and cause the
  // exact "lock:sb-... timed out" bug you were seeing.
  //
  // We only rely on:
  //  - the session restored at init()
  //  - auth state change events
  // If something looks stale, a user-initiated refresh fixes it, but we do not
  // spam getSession() in the background.
  return;
}


async function refreshCurrentPage(){
  // Re-run loaders when returning from another browser tab.
  // IMPORTANT: do not call auth.getSession() here (LockManager contention).
  const r = APP.route;
  try{
    if(r === 'home') await Promise.resolve(loadFeed()).catch(()=>{});
      ...
      if(uid) await Promise.resolve(loadProfileView(uid)).catch(()=>{});
      else await Promise.resolve(loadProfile()).catch(()=>{});
      await Promise.resolve(setProfileTab(APP.profileTab||'posts')).catch(()=>{});
    }
  }catch(_e){}
}
...
async function init(){
  ...
  // ---- Tab-return recovery (no LockManager spam) ----
  const _tabRecover = async ()=>{
    // If we already have a session token cached, ensure supabase auth client knows it.
    try{
      // If requests start behaving like anon after tab switch, force-set session from localStorage.
      const keys = Object.keys(localStorage || {}).filter(k=>k.startsWith('sb-') && k.includes('-auth-token'));
      let raw = null;
      for(const k of keys){
      ...
      if(raw){
        ...
        if(access_token && refresh_token){
          // update cached session (used by authFetch)
          APP.session = { ...(APP.session||{}), access_token, refresh_token, user: (APP.session?.user || j?.user || j?.currentSession?.user) };
          // Also inform supabase auth client (best effort)
          await APP.sb.auth.setSession({ access_token, refresh_token });
        }
      }
      // Re-hydrate profile + refresh current view (do not wipe on failure)
      await hydrateProfile({ preserve: true });
      refreshCurrentPage && refreshCurrentPage();
    }catch(_e){}
  };

  document.addEventListener('visibilitychange', ()=>{ if(!document.hidden) _tabRecover(); });
  window.addEventListener('focus', ()=>{ _tabRecover(); });
  window.addEventListener('pageshow', (e)=>{ if(e.persisted) _tabRecover(); });
  window.addEventListener('online', ()=>{ _tabRecover(); });
// initial route
  showPage('home');
// NOTE: Disabled focus/visibility auto-refresh.
// Chrome can trigger Navigator LockManager contention on Supabase auth storage when
// auto-refreshing on tab focus, which makes the UI appear empty until a hard refresh.
// We rely on onAuthStateChange and normal navigation to refresh views.
...
function wireUI(){
  ...
  document.addEventListener('click', (e)=>{
    ...
    // 2) Modal close
    if(t.closest?.('[data-closemodal]') || t.closest?.('#modalClose')){
      e.preventDefault();
      closeModal();
      return;
    }
    ...
    // 4) Booking browse: category/service/pager + request
    const catEl = t.closest?.('[data-cat]');
    if(catEl){
      APP.book.category = catEl.dataset.cat;
    ...
    if(svcEl){
      APP.book.service = svcEl.dataset.svc;
      renderBook().catch(()=>{});
      return;
    }
    const catpageEl = t.closest?.('[data-catpage]');
    if(catpageEl){
      const dir = catpageEl.dataset.catpage;
      if(dir === 'prev') APP.book.catPage = Math.max(0, (APP.book.catPage||0) - 1);
      if(dir === 'next') APP.book.catPage = (APP.book.catPage||0) + 1;
      renderBook().catch(()=>{});
      return;
    }
    ...
    if(rqEl){
      ...
      if(rqId) openRequestWizard(rqId).catch(()=>{});
      return;
    }

    // 5) Projects tabs + open detail
    ...
    if(profEl){
      // If they clicked a follow/unfollow button inside, let that handler run.
    ...
    // 8) Messages tabs/back/open thread
    ...
    if(confEl){
      ...
      if(id) confirmBooking(id).then(()=> openThread(id)).catch(()=>{});
      return;
    }
    ...
    // 9) Follow/unfollow
    const fEl = t.closest?.('[data-follow]');
    if(fEl){
      const id = fEl.dataset.follow;
      if(id) toggleFollow(id, true).catch(()=>{});
      return;
    }
    const unfEl = t.closest?.('[data-unfollow]');
    if(unfEl){
      const id = unfEl.dataset.unfollow;
      if(id) toggleFollow(id, false).catch(()=>{});
      return;
    }
  });
  ...
  if(btnMenu && userMenu){
    ...
    document.addEventListener('click', (e)=>{
      if(!userMenu.contains(e.target) && e.target !== btnMenu) userMenu.classList.remove('open');
    });
  }
...
async function ensureProfileRow({force=false} = {}){
  if(!APP.session?.user) return false;
  const uid = APP.session.user.id;

  // Avoid doing this constantly (it triggers auth token locks).
  if(!force && APP._profileEnsured) return true;

  // Serialize ensures
  if(APP._ensuringProfilePromise){
    try{ return await APP._ensuringProfilePromise; }catch(_e){ return false; }
  }

  APP._ensuringProfilePromise = (async ()=>{
    try{
      ...
      if(error){
        ...
        if(msg.includes('LockManager') || msg.includes('lock:sb-') || msg.includes('timed out')){
          console.warn('ensureProfileRow lock busy; will retry later');
          return false;
        }
        console.warn('ensureProfileRow', error.message, error.details, error.hint, error.code);
        return false;
      }
      APP._profileEnsured = true;
      return true;
    }catch(e){
      ...
      console.warn('ensureProfileRow', e);
      return false;
    }finally{
      // clear in-flight flag
      // (done in caller finally)
    }
  })();

  try{ return await APP._ensuringProfilePromise; }
  finally { APP._ensuringProfilePromise = null; }
}

async function hydrateProfile(opts={}){
  const preserve = !!opts.preserve;
  const prev = APP.me;
  if(!preserve) APP.me = null;
  if(!APP.session?.user){ if(preserve && !APP.me && prev) APP.me = prev; renderAuthBits(); return; }

  const uid = APP.session.user.id;
  if(!APP.profileView?.id) APP.profileView.id = uid;

  // Try to read profile first. Only upsert if missing.
  try{
    ...
    if(error){
      const msg = String(error?.message || error || '');
      if(msg.includes('LockManager') || msg.includes('lock:sb-') || msg.includes('timed out')){
        // keep existing UI; don't blank things out
        if(preserve && !APP.me && prev) APP.me = prev;
        renderAuthBits();
        return;
      }
      console.warn('hydrateProfile', error.message, error.details, error.hint, error.code);
      if(preserve && !APP.me && prev) APP.me = prev;
      if(preserve && !APP.me && prev) APP.me = prev;
      renderAuthBits();
      return;
    ...
    if(!data){
      // Profile row may be missing if the signup trigger hasn't run yet.
      // Don't upsert here (it can trigger auth-storage locks on some browsers).
      APP.me = { id: uid };
    }else{
      APP.me = data;
      APP._profileEnsured = true;
...
async function renderBook(){
  ...
  // Preserve matches on tab-return / visibility refresh if the selection hasn't changed.
  const key = `${chosenCatId||''}::${APP.book.service||''}`;
  if(APP._bookKey !== key){
    APP._bookKey = key;
    const m = byId('matches');
    const e = byId('matchesEmpty');
    if(m) m.innerHTML = '';
...
async function loadMessages(){
  ...
  const { data: bookings, error } = await APP.sb
    .from('bookings')
    .select(`
      id, status, created_at, updated_at, requested_date, total_cents, message, vision,
      requester, creator,
  ...
  const list = (bookings||[]).filter(b=>{
    ...
    // current threads = after accepted, plus completed
  ...
  const nameMap = {};
  if(otherIds.length){
    const { data: profs, error: pErr } = await APP.sb
      .from('profiles')
  ...
  if(threadsEl){
    threadsEl.innerHTML = list.map(b=>{
      ...
      const note = (b.vision || b.message) ? String(b.vision || b.message).trim() : '';
      ...
      const av = other.avatar_url
        ? `<img class="avimg" src="${esc(other.avatar_url)}" alt="">`
        : `<div class="avtxt">${esc((otherName||'u').replace('@','').slice(0,1).toUpperCase())}</div>`;
      return `
        <button class="threadCard" type="button" data-thread="${esc(b.id)}">
          <div class="tleft">
...
async function loadFollowStats(targetId){
  const tid = targetId || APP.profileView?.id || APP.session?.user?.id;
  if(!tid) return;
