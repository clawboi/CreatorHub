...
// Some browsers can leave a stale navigator.locks entry for Supabase auth storage.
// When that happens, ANY request that needs an access token can hang and then throw:
...
const __SB_REF__ = _supabaseProjectRef();
// Default storageKey is `sb-<ref>-auth-token` in supabase-js v2
const __SB_LOCK_NAME__ = __SB_REF__ ? `lock:sb-${__SB_REF__}-auth-token` : null;

async function _stealSupabaseAuthLock(reason){
  if(!__SB_LOCK_NAME__) return false;
  const locks = navigator.locks;
  if(!locks?.request) return false;
  if(APP._stealLockInFlight) return false;
  APP._stealLockInFlight = true;
  try{
    // "steal" is supported in modern Chromium. If not, this no-ops via catch.
    await locks.request(__SB_LOCK_NAME__, { mode:'exclusive', steal:true }, async ()=>{ /* immediately release */ });
    return true;
  }catch(_e){
    return false;
  }finally{
    APP._stealLockInFlight = false;
  }
}

function _isLockTimeout(err){
  const msg = String(err?.message || err || '');
  return msg.includes('Navigator LockManager') || msg.includes('lock:sb-') || msg.includes('timed out waiting');
}

async function _withAuthLockRecovery(fn, tries=2){
  let lastErr = null;
  for(let i=0;i<tries;i++){
    try{
      ...
      lastErr = e;
      if(!_isLockTimeout(e)) throw e;
      // Attempt to clear stale lock, then retry once
      await _stealSupabaseAuthLock('retry');
...
// Auto-heal: if any promise rejection is due to auth lock timeout, steal the lock once.
// This prevents the app from entering the "limbo" state after tab switching.
if(!window.__CH_LOCK_GUARD__){
  window.__CH_LOCK_GUARD__ = true;
  window.addEventListener('unhandledrejection', async (ev)=>{
    const r = ev?.reason;
    if(_isLockTimeout(r)){
      await _stealSupabaseAuthLock('unhandledrejection');
      toast('Auth storage unlocked. Try again.');
    }
  });
  window.addEventListener('error', async (ev)=>{
    if(_isLockTimeout(ev?.error || ev?.message)){
      await _stealSupabaseAuthLock('error');
    }
  });
}
'use strict';
...
async function ensureSessionFresh(){
  // Avoid auth.getSession() on tab focus (can deadlock LockManager).
  // Instead, rehydrate auth from localStorage token if needed.
  await recoverAuthIfNeeded("ensureSessionFresh");
}
...
function _getStoredAuthToken(){
  try{
    ...
    if(obj?.access_token && obj?.refresh_token) return { key:k, ...obj };
    if(obj?.currentSession?.access_token && obj?.currentSession?.refresh_token){
      return { key:k, access_token: obj.currentSession.access_token, refresh_token: obj.currentSession.refresh_token, expires_at: obj.currentSession.expires_at };
    }
    return null;
  }catch(e){ return null; }
}

function _buildSupabaseClient(){
  // SINGLETON: only create ONE Supabase client per browser context.
  // Creating multiple clients with the same storage key causes the exact tab-resume bug you see.
  if(window.__CH_SB__) return window.__CH_SB__;
  window.__CH_SB__ = window.supabase.createClient(window.SUPABASE_URL, window.SUPABASE_ANON_KEY, {
    auth: {
      persistSession: true,
...
function _rebindAuthListener(){
  try{ APP._authSub?.unsubscribe?.(); }catch(_e){}
  const res = APP.sb.auth.onAuthStateChange(async (event, session) => {
    if(event === 'SIGNED_OUT'){
      APP.session = null;
      APP.me = null;
    } else if(session){
    ...
    if(APP.route === 'signin' && APP.session?.user){
      showPage('home');
    }
  });
  APP._authSub = res?.data?.subscription || null;
}

async function recoverAuthIfNeeded(reason, opts={}){
  if(!APP.sb) return;
  if(APP._recoveringAuth) return;
  APP._recoveringAuth = true;
  try{
    // On resume: do NOT rebuild the Supabase client. That creates multiple GoTrue clients and breaks auth.
    // We keep a singleton client and only rehydrate session + refresh data.
const stored = _getStoredAuthToken();
    if(!stored) return;

    const haveUser = !!APP.session?.user?.id;
    const haveToken = !!APP.session?.access_token;
    const tokenChanged = haveToken && stored.access_token && (APP.session.access_token !== stored.access_token);

    // If everything looks good, still refresh page data on resume.
    if(haveUser && haveToken && !tokenChanged){
      await refreshCurrentPage();
      return;
    }

    // Try to refresh the session first (fresh JWT), then setSession from storage.
    try{
      if(APP.sb.auth.refreshSession){
        const rr = await APP.sb.auth.refreshSession();
        if(rr?.data?.session) APP.session = rr.data.session;
      }
    }catch(_e){ /* ignore */ }

    try{
      const res = await APP.sb.auth.setSession({ access_token: stored.access_token, refresh_token: stored.refresh_token });
      if(res?.data?.session) APP.session = res.data.session;
    }catch(_e){ /* keep going; do not blank state */ }

    // If setSession didn't return a session, keep token for gating and authFetch fallback.
    if(!APP.session && stored?.access_token){
      APP.session = APP.session || {};
      APP.session.access_token = stored.access_token;
    }

    await hydrateProfile({ preserve:true });
    await refreshCurrentPage();
  }finally{
    APP._recoveringAuth = false;
  }
}
...
async function init(){
  ...
  // Resume recovery: when returning to this browser tab, rehydrate auth from storage
  // and refresh current view. Avoid auth.getSession() to prevent LockManager contention.
  if(!APP._resumeBound){
  APP._resumeBound = true;
  const _onResume = ()=>{
    if(document.visibilityState === 'hidden') return;
    const now = Date.now();
    if(APP._lastResume && (now - APP._lastResume) < 800) return;
    APP._lastResume = now;
    recoverAuthIfNeeded('resume');
  };
  // visibility handler removed to prevent tab-freeze bug
...
async function hydrateProfile(opts={}){
  ...
  if(!APP.session?.user){ if(!preserve) APP.me = null; renderAuthBits(); return; }
...
async function loadProfileView(userId){
  ...
  // Fetch profile row
  const { data: prof, error } = await _withAuthLockRecovery(() =>
    APP.sb.from('profiles')
      .select('id,username,bio,avatar_url,featured_reel_url')
      .eq('id', userId)
      .maybeSingle()
  , 2);
...
document.addEventListener("visibilitychange", async () => {
  ...
  __resumeLock=true;
  try{
    // If the browser left a stale lock while we were backgrounded, clear it first.
    await _stealSupabaseAuthLock('visibilitychange');
    // Re-sync auth + current view without calling auth.getSession() directly.
    await _withAuthLockRecovery(()=>recoverAuthIfNeeded(), 2);
    // If we were on a view that needs data, refresh it once.
    await refreshCurrentPage();
  }catch(e){
    console.warn("resume flow failed", e);
  }finally{
    __resumeLock=false;
  }
});
