...
function wireUI(){
  ...
  document.addEventListener('click', (e) => {
    ...
    if(nav){
      ...
      // Auth-gated pages
      if((nav === 'book' || nav === 'create' || nav === 'projects' || nav === 'profile') && !APP.session?.user){
    ...
    // follow / unfollow (creator search)
    const followEl = t.closest?.('[data-follow]');
    const followId = followEl?.dataset?.follow;
    if(followId){
      toggleFollow(followId, true).catch(()=>{});
      return;
    }
    const unfollowEl = t.closest?.('[data-unfollow]');
    const unfollowId = unfollowEl?.dataset?.unfollow;
    if(unfollowId){
      toggleFollow(unfollowId, false).catch(()=>{});
      return;
    }
    ...
    // request package
    const reqEl = t.closest?.('[data-request]');
    const req = reqEl?.dataset?.request;
    if(req){
      e.preventDefault();
