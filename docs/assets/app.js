...
function refreshCurrentPage(){
  ...
  const r = APP.route;
  try{
    if(r === 'home') loadFeed().catch(()=>{});
    else if(r === 'book') renderBook().catch(()=>{});
    else if(r === 'projects') loadProjects().catch(()=>{});
    else if(r === 'messages') loadMessages().catch(()=>{});
    else if(r === 'profile') loadProfile().then(()=>setProfileTab(APP.profileTab||'posts')).catch(()=>{});
  }catch(_e){}
}
...
function showPage(name){
  ...
  if(name === 'messages'){
    ...
    APP.msgTab = APP.msgTab || 'current';
    // relabel tabs
    const p=byId('msgTabPending'); if(p) p.textContent='Current';
    const a=byId('msgTabApproved'); if(a) a.textContent='Completed';

    byId('msgTabPending')?.classList.toggle('is-on', APP.msgTab==='current');
    byId('msgTabApproved')?.classList.toggle('is-on', APP.msgTab==='completed');
    byId('msgListTitle') && (byId('msgListTitle').textContent = (APP.msgTab==='current') ? 'Current threads' : 'Completed threads');
    loadMessages().catch(()=>{});
  }
...
function wireUI(){
  ...
  document.addEventListener('click', (e) => {
    ...
    if(msgTabEl){
      ...
      APP.msgTab = tab || 'current';
      byId('msgTabPending')?.classList.toggle('is-on', APP.msgTab==='current');
      byId('msgTabApproved')?.classList.toggle('is-on', APP.msgTab==='completed');
      const ttl = byId('msgListTitle');
      if(ttl) ttl.textContent = (APP.msgTab==='current') ? 'Current threads' : 'Completed threads';
      loadMessages().catch(()=>{});
      return;
    }
...
async function openProjectDetail(bookingId){
  ...
  if(b.status === 'requested'){
    if(!isRequester){
      ...
      actions += `<button class="btn ghost" type="button" data-cancel="${esc(b.id)}">Reject</button>`;
    }else{
...
async function loadMessages(){
  ...
  const list = (bookings||[]).filter(b=>{
    const st = (b.status||'requested');
    if(APP.msgTab === 'current') return (st === 'accepted' || st === 'in_progress' || st === 'delivered');
    // completed
    return (st === 'completed');
  });
  ...
  if(!list.length){
    if(emptyEl){ emptyEl.style.display = 'block'; emptyEl.textContent = (APP.msgTab==='current') ? 'No current threads yet.' : 'No completed threads yet.'; }
