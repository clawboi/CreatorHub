    ...
    // Messages tabs/back
    const tabEl = t.closest?.('[data-msgtab]');
    if(tabEl){
      const tab = tabEl.dataset.msgtab;
      ...
      byId('msgListTitle').textContent = (APP.msgTab==='pending') ? 'Pending threads' : 'Approved threads';
      await loadMessages();
      return;
    }
    if(t.closest?.('#btnMsgBack') || t.closest?.('[data-backmsg]')){
      byId('msgDetailView').style.display = 'none';
      byId('msgListView').style.display = 'block';
    ...
    const cnl = t.closest?.('[data-cancel]');
    if(cnl){
      const id = cnl.dataset.cancel;
    ...
    if(conf){
      const id = conf.dataset.confirm;
      await confirmBooking(id);
      await openThread(id);
      return;
    }
    const gop = t.closest?.('[data-goproject]');
    if(gop){
      showPage('projects');
