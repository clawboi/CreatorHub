...
async function recoverAuthIfNeeded(reason, opts={}){
  ...
  try{
    // On tab resume, some browsers can leave the Supabase client in a stale in-memory state.
    // Rebuild the client from scratch (tokens remain in localStorage).
    if(opts.rebuildClient){
...
async function init(){
  ...
  const _onResume = ()=>{
    ...
    APP._lastResume = now;
    recoverAuthIfNeeded('resume', { rebuildClient:true });
  };
