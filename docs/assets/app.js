...
async function loadMessages(){
  ...
  threadsEl.innerHTML = bookings.map(b=>{
    ...
    const price = b.total_cents ? '$'+usdFromCents(b.total_cents) : '';
    ...
    return `
      <button class="threadCard ${on}" data-thread="${esc(b.id)}">
        <div class="tleft">
  ...
  const c = byId('msgThreadCount'); if(c){ c.style.display='inline-flex'; c.textContent = bookings.length+' threads'; }
...
async function openThread(bookingId){
  ...
  let peer = null;
  if(peerId){
    const { data: p } = await APP.sb.from('profiles').select('id,username,avatar_url,bio').eq('id', peerId).maybeSingle();
    peer = p || null;
  }
  ...
  if(nameEl) nameEl.textContent = peerId ? peerName : 'Select a thread';
  if(statsEl) statsEl.textContent = peerId ? `${followerCount} followers • ${followingCount} following` : '';
  if(avEl){
    ...
    avEl.style.cursor = peerId ? 'pointer' : '';
  }
  if(nameEl && peerId){
    nameEl.setAttribute('data-openprofile', peerId);
    nameEl.style.cursor = 'pointer';
  }


    const packTitle = b.packages?.title || 'Project';
  byId('msgMeta').textContent = `${packTitle} • ${b.status}${b.requested_date?` • preferred ${b.requested_date}`:''}${b.total_cents?` • $${usdFromCents(b.total_cents)}`:''}`;
  const sum = byId('msgSummary');
  if(sum){
    sum.style.display = 'block';
    sum.textContent = b.status==='requested' ? 'Request pending: accept to open chat.' : (b.status==='accepted' ? 'Accepted: both parties must confirm to lock it in.' : `Status: ${b.status}`);
  }
